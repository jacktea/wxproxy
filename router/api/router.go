...
func (this *ApiAction) InitRouter(app *bootstrap.Bootstrapper) {
	...
	// 代理微信接口
	r.Post("/proxyByDb/{componentAppId}/{apiId}", this.proxyByDb)
	r.Post("/proxyByDb/{componentAppId}/{appId}/{apiId}", this.proxyByDb)
...
func (this *ApiAction) proxyByDb(c iris.Context) {
	...
	if ok {
		...
		case "2":
			transPost(apiInfo.Url, token, context.ContentJSONHeaderValue, c.Request(), c)
		default:
